...
class DataSet:
    ...
    def plot_data(cls):
        ...
        plt.plot(x[0,y0_idx], x[1,y0_idx], 'rx', x[0,y1_idx], x[1,y1_idx], 'bo',)
        plt.axis('equal')    

    @classmethod
    def plot_decision_boundary(cls, w, extend_data=None, norm_param=None):
        h = .05 
        y,x = cls.get_data()
        ...
        if norm_param is not None:
            ...
            xx = (xx - mean)/stdd            
        a = w.T.dot(xx).reshape(x0_grid.shape)
        cls.plot_data()
        plt.contour(x0_grid, x1_grid, a, 0) 
        
    @classmethod
    def save_decision_boundary(cls, w, extend_data=None, norm_param=None):
        h = .05 
        y,x = cls.get_data()
        x0_min, x0_max = x[0,:].min() - 1, x[0,:].max() + 1
        x1_min, x1_max = x[1,:].min() - 1, x[1,:].max() + 1
